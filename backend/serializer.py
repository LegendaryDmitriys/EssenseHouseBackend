...
from .models import House, ConstructionTechnology, Filter, HouseCategory, HouseImage, HouseInteriorImage, \
    HouseFacadeImage, HouseLayoutImage, FinishingOption, Document, Review, Order
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def validate_construction_place(self, value):
        # Проверяем, что значение начинается с "Новгородская область"
        ...
        # Теперь отправляем запрос к OpenStreetMap с полным адресом
        geocode_url = 'https://nominatim.openstreetmap.org/search'
        params = {'q': value, 'format': 'json'}  # Передаем полное значение
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if geo_data:
                    # Извлекаем информацию из display_name
                    display_name = geo_data[0].get('display_name', '')
                    print(f"Полное название: {display_name}")

                    # Проверяем наличие "Новгородская область" в display_name
                    if 'новгородская область' not in display_name.lower():
                        print("Регион не соответствует требованиям.")  # Логируем предупреждение
